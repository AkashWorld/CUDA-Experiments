...
void cublas_matrix_multiplication(const std::size_t size, const unsigned int iteration)
{
	fprintf(stdout, CYN(BOLD("[CUBLAS]") " Matrix multiplication of " YEL("%lux%lu") " Matrices for " YEL("%u") " iterations...\n"), size, size, iteration);
	fprintf(stdout, BOLD(RED("Calculating...")));
	fflush(stdout);
	auto lh_mat = Matrix<unsigned int>::generate_random_matrix(size, size);
	auto rh_mat = Matrix<unsigned int>::generate_random_matrix(size, size);
...
int main(int argc, char **argv)
{
	...
	cublas_matrix_multiplication(100, 500);
	cublas_matrix_multiplication(100, 1000);
	cublas_matrix_multiplication(100, 1500);
